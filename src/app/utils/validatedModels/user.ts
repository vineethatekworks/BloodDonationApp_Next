...
export const UserSchema = z.object({
  ...
  contact: z.string().min(10, { message: 'Phone number must be at least 10 digits long' }).regex(/^\d+$/, { message: 'Phone number must contain only digits' }).optional(),
  blood_group: z.enum(['A+', 'A-', 'B+', 'B-', 'AB+', 'AB-', 'O+', 'O-']).optional().transform(val => val ? bloodGroupMap[val as string] : undefined),
  location: z.string().min(2, { message: 'Location must be at least 2 characters long' }).optional(),
  role: z.enum(['DONOR', 'RECIPIENT', 'ADMIN']),
  available_to_donate: z.boolean().optional(),
